...
void MainWindow::initLogging() {
    ...
    if (m_laser_logFile.open(QIODevice::WriteOnly | QIODevice::Text)) {
        ...
        m_laser_logStream << "Timestamp,MinDistance [m],AvgDistance [m],AvgCenterDistance "
                             "[m],Timer [min:s.100ms],linear velocity (x)"
                             "[m/s],linear velocity"
...
void MainWindow::logEvent() {
    ...
    float avgCenterDist = m_ui->obstacle_map_widget->getAvgCenterDistance();
    ...
    if (m_laser_logFile.isOpen()) {
        m_laser_logStream << QDateTime::currentDateTime().toString(Qt::ISODate) << "," << min << ","
                          << avgDist << "," << avgCenterDist << "," << only_time_text << "," << vel.linear.x << ","
