...
void CustomTouchSliderVertical::paintEvent(QPaintEvent*) {
    ...
    if (MainWindow::s_dynLock) {
        ...
        painter.drawRect(width() / 2 - sliderWidth / 2, deadzone_top, sliderWidth, deadzone_height);
    }
}
        ...
        if (event->type() == QEvent::TouchBegin || event->type() == QEvent::TouchUpdate) {
            for (const QTouchEvent::TouchPoint& point : touchPoints) {
                ...
                if (rect().contains(touchPos.toPoint())) { // Überprüfen, ob der Touchpunkt im
                    ...
                    double newValue =
                        std::clamp(mapToSliderValue(touchPos.y()), -1.0,
...
void CustomTouchSliderVertical::lockIn(double absValue) {
